...
def test_user_can_see_own_data_without_house_permissions() -> None:
    scope = VisibilityService().build_scope(user_id=1, memberships=[])

    assert scope.visible_user_ids == [1]
    assert scope.visible_house_ids == []


def test_house_permission_includes_house_users_and_house() -> None:
    ...
    assert scope.visible_user_ids == [1, 2, 3]
